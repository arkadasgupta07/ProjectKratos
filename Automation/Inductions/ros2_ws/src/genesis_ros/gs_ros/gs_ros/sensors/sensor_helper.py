...
def raycaster_to_laser_scan_msg(
    ...
    ranges = data.squeeze().tolist()
    cleaned_ranges = []
    for v in ranges:
        if np.isnan(v) or np.isinf(v) or v > far:
            cleaned_ranges.append(float(far))
        elif v < near:
            cleaned_ranges.append(float(near))
        else:
            cleaned_ranges.append(float(v))
    msg.ranges = cleaned_ranges
