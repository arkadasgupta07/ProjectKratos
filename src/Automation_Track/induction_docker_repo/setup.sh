...
# ── Step 2: NVIDIA GPU ──
step "Step 2/3: NVIDIA GPU Support"
if command -v nvidia-smi &>/dev/null; then
    ...
    info "No NVIDIA GPU — GUI apps will use software rendering (still works)"
fi

# ── Step 3: Pull image ──
step "Step 3/3: Container Image"
info "Pulling $IMAGE (may take a while, ~4-6 GB)..."
$DOCKER pull --platform linux/amd64 "$IMAGE" || fail "Failed to pull image. Check internet or build locally: docker build -t $IMAGE ."
ok "Image ready"

echo ""
echo -e "${GREEN}${BOLD}  Setup complete ✓  Run: ./kratos-env.sh start${NC}"
echo ""
