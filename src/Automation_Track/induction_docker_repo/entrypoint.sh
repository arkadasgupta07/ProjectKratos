...
# Creates a dev user, starts the VNC + noVNC stack, and sources ROS2.
...
mkdir -p "$HOME/.vnc" "$HOME/.config/fluxbox" "$HOME/.ros"
chown -R "$HOST_UID:$HOST_GID" "$HOME/.vnc" "$HOME/.config" "$HOME/.ros" 2>/dev/null || true
...
# Done — keep alive
log "Container running. Attach with: ./kratos-env.sh shell"
wait $VNC_PID
